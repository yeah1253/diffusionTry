...
class PhysiNet(Module):
    ...
    def __init__(
        ...
        if self.legacy_cond:
            # ========== 旧版条件注入：cond_mlp 直接输出到 time_dim ==========
            # cond_mlp: Linear(cond_dim→time_dim) → GELU → Linear(time_dim→time_dim)
            if self.cond_dim > 0:
        ...
        # 旧版 checkpoint 没有此模块，legacy_cond=True 时跳过
        if self.cond_dim > 0 and not self.legacy_cond:
    ...
    def forward(self, x, time, cond: Tensor = None, phys_signal: Tensor = None, x_self_cond = None):
        ...
        # 4) 条件注入（legacy 旧版 / 新版 Class Token Transformer 两条路径）
        if self.cond_dim > 0 and cond is not None and exists(self.cond_mlp):
            ...
            if self.legacy_cond:
                # ---- 旧版：cond_mlp 直接输出 time_dim，加到 t ----
                cond_emb = self.cond_mlp(cond_vec)          # (B, time_dim)
                t = t + cond_emb.to(t.dtype)
            else:
                # ---- 新版：Class Token + Transformer ----
                # ---- 物理先验提升层 ----
